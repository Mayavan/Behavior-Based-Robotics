# -*- coding: utf-8 -*-
"""
Created on Tue Sep 19 16:40:17 2017
...
def threaded_function():
    while True:
        errorCode, detectionState1, detectedPoint1, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, front_left, vrep.simx_opmode_streaming)
        if debug:
            print detectionState1
        errorCode, detectionState2, detectedPoint2, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, front_right, vrep.simx_opmode_streaming)
        if debug:
            print detectionState2
        errorCode, detectionState3, detectedPoint3, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, sLeft, vrep.simx_opmode_streaming)
        if debug:
            print detectionState3
        errorCode, detectionState4, detectedPoint4, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, sRight, vrep.simx_opmode_streaming)
        if debug:
...
if clientID != -1:
    ...
    errorCode, front_right = vrep.simxGetObjectHandle(clientID, "Front_Right",
                                                      vrep.simx_opmode_oneshot_wait)
    errorCode, sLeft = vrep.simxGetObjectHandle(clientID, "Left",
                                                vrep.simx_opmode_oneshot_wait)
    errorCode, sRight = vrep.simxGetObjectHandle(clientID, "Right",
                                                 vrep.simx_opmode_oneshot_wait)
