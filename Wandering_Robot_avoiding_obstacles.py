...
if clientID != -1:
    ...
    while True:
        ...
        print "Front Right Reading :{}\n".format(
            int(detectedPoint2[2] * 100) if detectionState2 else "Nothing in Range")
        ...
        if Turning:
            r = randint(0, 100)
            print "Stop Turn Probability :{}%\n".format(r)
            if r < StopTurnProbability:
                LeftMotorSignal = Speed
                RightMotorSignal = Speed
                Turning = False
        else:
            r = randint(0, 100)
            print "Turn Probability :{}%\n".format(r)
            if r < TurnProbability:
                s = randint(0, 100)
                print "Direction Probability :{}%\n".format(r)
                if s < DirectionProbability:
                    LeftMotorSignal = Speed
                    RightMotorSignal = -Speed
                else:
                    LeftMotorSignal = -Speed
